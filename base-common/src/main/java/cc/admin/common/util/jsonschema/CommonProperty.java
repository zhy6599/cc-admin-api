package cc.admin.common.util.jsonschema;
...
public abstract class CommonProperty implements Serializable{
	...
	/**
	 * <getPageFilterFields>此关键字的值必须是字符串或数组。如果它是一个数组，那么数组的元素必须是字符串，并且必须是唯一的。
	 * <getPageFilterFields>字符串值必须是六种基本类型之一（“null”，“boolean”，“object”，“array”，“number”或“string”），或“integer”，它匹配任何数字，零分数部分。
	 * <getPageFilterFields>当且仅当实例位于为此关键字列出的任何集合中时，实例才会验证。
     *
	 */
	...
	/**
	 * 对应JsonSchema的enum
	 * <getPageFilterFields>该关键字的值必须是一个数组。这个数组应该至少有一个元素。数组中的元素应该是唯一的。如果实例的值等于此关键字的数组值中的某个元素，则实例将对此关键字成功验证。
	 * 数组中的元素可以是任何值，包括null
	 *
	 *  {
	 *   "type": "string",
	...
	/**
	 * 对应JsonSchema的const
	 * <getPageFilterFields>此关键字的值可以是任何类型，包括null。
