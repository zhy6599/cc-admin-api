package cc.admin.modules.quartz.controller;
...
public class QuartzJobController {
	...
	public Result<?> queryPageList(QuartzJob quartzJob,
		...
		if (StrUtil.isNotEmpty(key)) {
			queryWrapper.or().like("job_class_name", key);
			queryWrapper.or().like("parameter", key);
	...
	public Result<Object> pauseJob(@RequestParam(name = "jobClassName", required = true) String jobClassName) {
		...
		try {
			...
			throw new CcAdminException("暂停定时任务失败");
		}
	...
	public ModelAndView exportXls(HttpServletRequest request, QuartzJob quartzJob) {
		...
		mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("定时任务列表数据", "导出人:", "导出信息"));
