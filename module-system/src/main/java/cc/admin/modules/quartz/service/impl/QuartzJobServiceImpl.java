package cc.admin.modules.quartz.service.impl;

import java.util.List;

import cc.admin.modules.quartz.entity.QuartzJob;
import cc.admin.modules.quartz.mapper.QuartzJobMapper;
...
public class QuartzJobServiceImpl extends ServiceImpl<QuartzJobMapper, QuartzJob> implements IQuartzJobService {
	...
	private void schedulerAdd(String jobClassName, String cronExpression, String parameter) {
		try {
			...
			throw new CcAdminException("创建定时任务失败", e);
		} catch (RuntimeException e) {
			throw new CcAdminException(e.getMessage(), e);
		}catch (Exception e) {
			throw new CcAdminException("后台找不到该类名：" + jobClassName, e);
		}
	}
	...
	private void schedulerDelete(String jobClassName) {
		try {
			...
			throw new CcAdminException("删除定时任务失败");
		}
	}
